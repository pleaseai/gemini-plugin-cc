  ...
  try {
    switch (message.method) {
      case "initialize":
        ...
        saveState(state);
        send({ id: message.id, result: { userAgent: "fake-gemini-app-server" } });
        break;
      ...
      case "thread/start": {
        ...
        send({ id: message.id, result: { thread: buildThread(thread), model: message.params.model || "gemini-3-flash-preview", modelProvider: "google", serviceTier: null, cwd: thread.cwd, approvalPolicy: "never", sandbox: { type: "readOnly", access: { type: "fullAccess" }, networkAccess: false }, reasoningEffort: null } });
      ...
      case "review/start": {
        ...
        const payload = message.params.outputSchema && message.params.outputSchema.properties && message.params.outputSchema.properties.verdict
          ...
          : taskPayload(prompt, thread.name && thread.name.startsWith("Gemini Companion Task") && prompt.includes("Continue from the current thread state"));
...
`;
  ...
  // Create a gemini.cmd so the fake binary is discoverable by spawn with shell: true.
