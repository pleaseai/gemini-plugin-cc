...
import { BROKER_BUSY_RPC_CODE, BROKER_ENDPOINT_ENV, GeminiAppServerClient } from "./app-server.mjs";
...
function applyTurnNotification(state, message) {
  switch (message.method) {
    ...
    case "error":
      ...
      emitProgress(state.onProgress, `Gemini error: ${message.params.error.message}`, "failed");
      break;
    case "turn/completed":
      if ((message.params.threadId ?? null) !== state.threadId) {
...
export function getGeminiAvailability(cwd) {
  ...
  const appServerStatus = binaryAvailable("gemini", ["app-server", "--help"], { cwd });
...
export async function interruptAppServerTurn(cwd, { threadId, turnId }) {
  ...
  try {
    client = brokerEndpoint
      ? await GeminiAppServerClient.connect(cwd, { brokerEndpoint })
      : await GeminiAppServerClient.connect(cwd, { disableBroker: true });
...
export async function runAppServerReview(cwd, options = {}) {
  ...
  return withAppServer(cwd, async (client) => {
    emitProgress(options.onProgress, "Starting Gemini review thread.", "starting");
...
export async function runAppServerTurn(cwd, options = {}) {
  ...
  return withAppServer(cwd, async (client) => {
    ...
    if (options.resumeThreadId) {
      ...
      emitProgress(options.onProgress, "Starting Gemini task thread.", "starting");
