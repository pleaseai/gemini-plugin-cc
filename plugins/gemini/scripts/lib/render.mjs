...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.threadId) {
    lines.push(`  Gemini CLI session ID: ${job.threadId}`);
  }
...
export function renderReviewResult(parsedResult, meta) {
  if (!parsedResult.parsed) {
    const lines = [
      `# Gemini CLI ${meta.reviewLabel}`,
      "",
      "Gemini CLI did not return valid structured JSON.",
      "",
  ...
  if (validationError) {
    const lines = [
      ...
      "Gemini CLI returned JSON with an unexpected review shape.",
      "",
...
export function renderTaskResult(parsedResult, meta) {
  ...
  const message = String(parsedResult?.failureMessage ?? "").trim() || "Gemini CLI did not return a final message.";
  return `${message}\n`;
}
...
export function renderJobStatusReport(job) {
  const lines = ["# Gemini CLI Job Status", ""];
  pushJobDetails(lines, job, {
...
export function renderStoredJobResult(job, storedJob) {
  ...
  if (isStructuredReviewStoredResult(storedJob) && storedJob?.rendered) {
    ...
    return `${output}\nGemini CLI session ID: ${threadId}\nResume in Gemini: ${resumeCommand}\n`;
  }
  ...
  if (threadId) {
    lines.push(`Gemini CLI session ID: ${threadId}`);
    lines.push(`Resume in Gemini: ${resumeCommand}`);
  }
